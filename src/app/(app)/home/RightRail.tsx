    ...
    <div className="space-y-4">
          ...
          <ul className="divide-y divide-hairline">
            {top.map((a) => (
              <li key={a.id} className="py-3 first:pt-0 last:pb-0">
                {a.pinnedAt && <span className="mb-1 inline-block rounded-none bg-red/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.1em] text-red">Pinned</span>}
      ...
      <section className="rounded-none border border-hairline bg-surface p-5">
        <div className="flex items-start gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-none bg-red/10 text-red">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 flex items-center gap-2 text-[12px] text-ink-secondary">
              10:00 AM – 11:00 AM
              <span className="rounded-none bg-red/10 px-1.5 py-0.5 text-[9px] font-bold uppercase text-red">Free</span>
            </p>
          </div>
        </div>
        <Link href="/referral-rounds" className="mt-3 block rounded-none border border-red/30 bg-red/5 px-3 py-2 text-center text-[12px] font-semibold text-red transition-colors hover:bg-red/10">View Details →</Link>
      </section>
          ...
          <ul className="space-y-3">
            {people.map((p) => (
              <li key={p.slug} className="flex items-center gap-3">
                ...
                <Link href={`/m/${p.slug}`} className="shrink-0 rounded-none border border-hairline px-3 py-1.5 text-[11px] font-semibold text-ink transition-colors hover:border-hairline-bright">View Profile</Link>
              </li>
            ))}
          </ul>
        </section>
